...
export default (WebSocket => class Client extends EventEmitter {
    ...
    constructor(address = "ws://localhost:8080", {
        ...
        if (this.autoconnect) this._connect(address, arguments[1]);
    }
    ...
    async subscribe(event) {
        ...
        if (result[event] !== "ok") throw new Error("Failed subscribing to an event '" + event + "' with: " + result[event]);
    }
    ...
    async unsubscribe(event) {
        ...
        if (result[event] !== "ok") throw new Error("Failed unsubscribing from an event with: " + result);
    }
